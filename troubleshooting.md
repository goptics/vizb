...
**Fix:** Make `-p` mirror `-g` — same separator characters in the same positions. See [Matching separators](/guides/group/#matching-separators).
...
**Fix:** Count top-level slots in `-p` (each `[...]` bracket counts as one slot). They must match the number of `-g` columns. See [Bracket slots](/guides/group/#bracket-slots--split-a-columns-cell-value).
...
**Fix:** Check spelling and run `vizb data.csv -o out.html` without `-g` first to confirm headers. See [Grouping CSV/JSON data](/guides/group/#group-csvjson-data).
...
**Fix:** Add `x` or `y` to the pattern, e.g. `-p n/x/y` instead of `-p n`. See [Dimensions](/getting-started/dimensions).
...
**Fix:** Add `(?<x>...)` or `(?<y>...)` to the regex. See [Regex Syntax](/guides/group/#regex-syntax).
