...
func BenchmarkApplyValidationRules(b *testing.B) {
	value := validFormat1
	rule := ValidationRule{
		Label:    "format",
		Value:    &value,
...
func BenchmarkApplyValidationRulesWithNormalizer(b *testing.B) {
	value := "JSON"
	rule := ValidationRule{
		Label:      "format",
		Value:      &value,
...
func BenchmarkIsBenchJSONFile(b *testing.B) {
	tempDir := b.TempDir()
	content := `{"Action":"run","Test":"BenchmarkExample"}
{"Action":"pass","Test":"BenchmarkExample","Output":"result"}
`
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		IsBenchJSONFile(filePath)
	}
}

func BenchmarkIsBenchJSONFileLarge(b *testing.B) {
	tempDir := b.TempDir()
	...
	filePath := filepath.Join(tempDir, "large_bench.json")
	err := os.WriteFile(filePath, []byte(content.String()), 0644)
	if err != nil {
		b.Fatal(err)
	}
