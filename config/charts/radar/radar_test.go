...
func TestMaterialise_RadarPrecedence(t *testing.T) {
	tr := true
	fa := false
...
// radar has no Scale or AutoRotate fields.
func TestRadarConfig_NoScaleOrAutoRotate(t *testing.T) {
	typ := reflect.TypeOf(Config{})
	_, hasScale := typ.FieldByName("Scale")
	_, hasAutoRotate := typ.FieldByName("AutoRotate")
	assert.False(t, hasScale, "radar Config should not have a Scale field")
	assert.False(t, hasAutoRotate, "radar Config should not have an AutoRotate field")
}
