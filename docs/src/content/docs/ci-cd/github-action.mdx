...
| `chart` | `""` | Per-chart overrides (`--chart` flag, repeatable). One override per line: `<type>:<key>=<val>,...`. Keys include `swap`, `sort`, `scale`, `labels`, `label-mode`, `3d-rotate`, `3d`. E.g. `bar:label-mode=percentage`. Blank lines and `#`-prefixed lines are ignored. |
...
Available override keys include `swap`, `sort`, `scale`, `labels`, `label-mode`, `3d-rotate`, `3d`.
