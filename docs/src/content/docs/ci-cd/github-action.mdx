...
<Aside type="caution">
  At least one of `bench-cmd`, `bench-file`, `merge-files`, `merge-dir`, or `data-url` must be provided.
</Aside>
...
<Aside type="note">
  When `data-url` is set, no `bench-cmd`, `bench-file`, or merge input is needed. The HTML fetches the JSON at runtime. The JSON host must serve `Access-Control-Allow-Origin: *`.
</Aside>
