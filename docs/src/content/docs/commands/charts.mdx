...
Chart subcommands (`vizb bar`, `vizb line`, `vizb scatter`, `vizb pie`, `vizb heatmap`, `vizb radar`, `vizb sankey`, `vizb chord`) generate one chart at a time. They produce the same output as the root command with `--charts` limited to that type, but accept only the flags valid for the chart — unsupported flags fail fast.
...
`<chart>` is one of `bar`, `line`, `scatter`, `pie`, `heatmap`, `radar`, `sankey`, `chord`. Output is HTML by default, or JSON when `-o` ends in `.json` (same rules as the root command).
