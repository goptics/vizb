...
| `--group-pattern` | `-p` | `x` | Pattern-based grouping (`n`/`x`/`y`/`z`) |
| `--group-regex` | `-r` | `""` | Regex-based grouping (named captures) |
| `--group` | `-g` | `""` | Names each dimension in `--group-pattern` order |
...
vizb bar data.csv -p n/x/y/z --scale log --3d-rotate -o bar3d.html
