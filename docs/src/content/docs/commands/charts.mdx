...
Each chart type has its own subcommand: `vizb bar`, `vizb line`, `vizb scatter`, `vizb pie`, `vizb heatmap`, and `vizb radar`. A chart subcommand is equivalent to the root command restricted to one chart — `vizb bar data.csv` produces the same output as `vizb data.csv --charts bar` — but it exposes **only the flags valid for that chart**. For example, `--scale` and `--3d-rotate` exist on `bar`/`line`/`scatter` but not on `pie`/`heatmap`/`radar`, and only `scatter` accepts `--axes`, so an unsupported flag fails fast instead of being silently ignored.
...
| `--axes` | — | — | ✅ | — | — | — | csv/json only: 2–3 numeric columns as raw coordinates, or 1 column with exactly 2 group dims (hybrid) |

`bar`, `line`, and `scatter` render in 3D when the data has a z dimension (`-p n/x/y/z`) or scatter uses `--axes` with three columns. Pie, heatmap, and radar are 2D-only for those flags — passing an unsupported flag is an error:
...
# Scatter from raw numeric columns (value mode)
vizb scatter data.csv --axes price,latency -o scatter.html
