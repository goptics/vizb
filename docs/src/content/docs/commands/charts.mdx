...
| `--theme` | | *(empty)* | Embed a color theme on the dataset (**repeatable**). Built-in name, structured `name:colors=#hex,...;visualMapColors=#hex,#hex`, or bare `#hex,#hex,...`; see [Color Themes](/ui/themes) |
| `--theme-active` | | `""` | Name of the active theme (moved first in the embedded catalog) |
