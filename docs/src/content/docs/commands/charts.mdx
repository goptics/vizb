...
| `--rotate` | ✅ | ✅ | — | — | — | Auto-rotate the 3D chart (only meaningful with z-axis data) |

`bar` and `line` render in 3D when the data has a z dimension (`-p n/x/y/z`), which is why they accept `--scale` and `--rotate`. The other chart types are 2D-only, so those flags are intentionally absent — passing them is an error:
...
vizb bar data.csv -p n/x/y/z --scale log --rotate -o bar3d.html
