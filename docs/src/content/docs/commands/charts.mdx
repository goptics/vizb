...
Chart subcommands (`vizb bar`, `vizb line`, `vizb scatter`, `vizb pie`, `vizb heatmap`, `vizb radar`, `vizb sankey`) generate one chart at a time. They produce the same output as the root command with `--charts` limited to that type, but accept only the flags valid for the chart — unsupported flags fail fast.
...
`<chart>` is one of `bar`, `line`, `scatter`, `pie`, `heatmap`, `radar`, `sankey`. Output is HTML by default, or JSON when `-o` ends in `.json` (same rules as the root command).

```bash
vizb bar data.csv -g impl,size -p n,x -o bar.html
vizb pie data.csv -g impl -o pie.html
vizb sankey data.csv -g from,to -p x,y -o sankey.html
...
| Flag | `bar` | `line` | `scatter` | `pie` | `heatmap` | `radar` | `sankey` | Description |
|------|:---:|:---:|:---------:|:---:|:---:|:---:|:---:|-------------|
...
`bar`, `line`, and `scatter` render in 3D when the data has a z dimension (`-p n/x/y/z`) or auto-value detects 3+ numeric columns. Pie, heatmap, radar, and sankey are 2D-only for those flags — passing an unsupported flag is an error:
...
vizb sankey examples/csv/sankey-flows.csv -g from,to -p x,y -o sankey.html
```
