...
- **Object props** use braces: `key={field=value;field=value}`. Inside `{}`,
  ...
  `borderRadius=8,8,0,0`). Unwrapped `bar:bg=color=…` or
  `line:scale=type=log;axes=x` is invalid. Nested `log:axes=x` is not valid.
...
# Per-axis log object (braces; commas inside {} stay literal)
vizb bench.txt -c line --chart 'line:scale={type=log;axes=x}' -o output.html
vizb bench.txt -c line --chart 'line:scale={type=log;axes=x,y;baseX=5;baseY=10};smooth' -o output.html
