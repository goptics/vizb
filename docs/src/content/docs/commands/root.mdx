...
| `--charts` | `-c` | `bar,line,pie,heatmap,radar` | Chart types to generate (`bar`, `line`, `pie`, `heatmap`, `radar`) |
...
By default all five chart renderers are bundled. Use `--charts` to include only
the ones you need — unselected renderers are dropped at generation time.
...
vizb bench.txt -p n/x/y/z --chart bar:rotate --chart line:labels=false -o output.html
```
...
| `rotate` | `rotate` | `true`, `false` | `bar`, `line` 3D only |
