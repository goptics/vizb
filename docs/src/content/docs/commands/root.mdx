...
| `--theme` | | *(empty)* | Embed a color theme on the dataset (**repeatable**). Built-in name, structured `name:colors=#hex,...;visualMapColors=#hex,#hex`, or bare `#hex,#hex,...` palette. Empty when unset (UI default). Built-in `default` is not embedded. |
...
# Multiple themes (selector shown); pin active with --theme-active
vizb data.csv --theme westeros --theme vintage --theme-active vintage -o report.html
