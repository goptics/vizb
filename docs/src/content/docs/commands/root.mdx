...
|------|---------|-------------------|--------------|
| **Grouped stat pick** | `-g`, `-r`, or non-default `-p` | Any number of numeric columns; optional `{label}` | Picks which numeric columns become chart series |
...
| **Solo mixed axes** | `--select` only; categorical `x` + numeric `y[,z]` | 2–3 cols per flag: `region,latency` | Category on x, values on y/z; [scatter](/charts/scatter) is the primary chart |
...
- Each flag value needs **2–3 columns** (assigned to `x`, `y`, optional `z` by position).
- **Explicit placement:** `x:region,y:latency,z:sales` — use for every column or omit prefixes for all.
- **Repeatable** for multi-view output: each `--select` produces one dataset; HTML embeds all views (tabs in the UI).
- **No overlap** with `--group` columns.
...
# Multi-view: two datasets in one HTML
...
JSON output is a single dataset object when there is one view, or a **JSON array** when `--select` is repeated. Dataset names auto-generate as `region × latency`, `region × sales`, etc. With `--id`, additional views get suffixed ids (`my-id-2`, …).

See [Grouping → Choosing: `--group` + select vs solo `--select`](/guides/grouping#choosing---group--select-vs-solo---select) for when to use each approach.
