...
`--theme` changes series colors only; light/dark mode remains independent. Viewers can switch
built-in palettes from the generated report, and their choice is saved in the browser. For a
brand palette, pass at least two `#rgb` or `#rrggbb` colors:
