...
Vizb turns any CSV or JSON table into interactive HTML charts. It also reads benchmark output from Go, Rust, and JavaScript. A benchmark result is just a table of named, numeric rows. Everything below works the same way for both tables and benchmarks.
...
<CardGrid stagger>
  ...
  <Card title="Smart Filtering" icon="magnifier">
    Include only matching rows with `--filter` regex pattern.
  </Card>
