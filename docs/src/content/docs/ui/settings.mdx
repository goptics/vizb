...
# Per-axis log bag / --chart braces (panel still only Linear / Logarithmic)
vizb line data.csv --scale 'type=log;axes=x' -o output.html
vizb data.csv -c line --chart 'line:scale={type=log;axes=x;base=10}' -o output.html
vizb data.csv -c line --chart 'line:scale={type=log;axes=x,y;baseX=5;baseY=10};smooth' -o output.html
