...
Other common params: `?c=` (active chart type), `?g=` (group index), and per-chart keys like `bar.so=desc`, `line.sc=log`, and `bar.lm=percentage`. Label modes use `<chart>.lm=none|value|percentage`. Legacy global `l=true|false` and per-chart `<chart>.l=true|false` URLs remain value-label inputs; `<chart>.lm` wins when both are present. See the [root command](/commands/root#deep-links) for the `--id` flag.
