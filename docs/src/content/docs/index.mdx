---
title: Vizb
description: Turn any CSV or JSON table into interactive 4D charts in one HTML file. Also supports benchmark output from Go, Rust, and JavaScript.
template: splash
hero:
  tagline: Any CSV or JSON table → interactive 4D charts. One self-contained HTML file. No server, no build step.
  image:
...
<CardGrid stagger>
  ...
  <Card title="Statistics Panel" icon="approve-check">
    Per-series descriptive stats and a Pearson/Spearman correlation matrix. Computed off-thread so the UI stays responsive.
  </Card>
