...
7. **Prune** — `SelectChunks` (`pkg/template/chunks.go`) walks the chunk reference
   ...
   chart renderers. The 3D engine is gated separately: included when `--charts`
   contains `bar` or `line` and data has a z-axis.
8. **Output** — JSON (`json.Marshal`) or HTML (Vue template + embedded JSON + pruned chunk map)
