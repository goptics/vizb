...
WinGet is available now. Homebrew is coming soon — we are [looking for a contributor](https://github.com/goptics/vizb/issues/205) to add a formula.

<PackageTabs disabled={['Homebrew']}>
  <Tabs>
    <TabItem label="WinGet" icon="seti:windows">
    ```ps1
    winget install --id goptics.vizb --exact
    ```
    </TabItem>
