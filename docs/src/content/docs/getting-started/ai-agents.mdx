...
npx skills add goptics/vizb -g

# confirm the CLI can see it
npx skills add goptics/vizb --list
```

The [skills CLI](https://github.com/vercel-labs/skills) copies `skills/vizb` into the agents you have installed (Grok, Claude Code, Cursor, Codex, and others). After that, `/vizb` is available. Automatic invocation also stays on: “chart this CSV” or “plot the Go bench” should load the skill without typing `/vizb`.
