...
<InvokeTabs
  cli={`vizb line sales.csv -g order_date -p x -o out.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb line sales.csv -g region,category -p x,y -o out.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb line sales.csv -g region,category -p x,y --stack -o stacked-area.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb line worker-pools.txt -p z/y/x --scale log -o out.html`}
/>
...
<InvokeTabs
  cli={`vizb line sales.csv -g region,product -p x,y --select quantity,amount -o lines.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb line spiral-3d.csv -o line.html --3d-visualmap --3d-rotate`}
/>
...
| Scale (log) | `--scale log` or `--scale 'type=log;axes=x'` | Scale toggle | `linear` / `log` (value axis), or bag for per-axis log. Panel is Linear / Logarithmic only (cannot pick axes or base; hover is read-only). Gaps at ≤0 |
...
vizb line sales.csv --scale 'type=log;axes=x,y;base=10' -o out.html

# --chart braces (root multi-chart path)
vizb sales.csv -c line --chart 'line:scale={type=log;axes=x;base=10}' -o out.html
vizb sales.csv -c line --chart 'line:scale={type=log;axes=x,y;baseX=5;baseY=10};smooth' -o out.html
