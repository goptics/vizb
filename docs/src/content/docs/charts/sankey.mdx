...
# Solo --select: same order — source, target, value (min 3 columns; no -g)
...
|----------|---------------------------|--------|
| **Group** | `-g from,to -p x,y` | source/target as group axes; remaining numeric columns become measures |
| **Solo `--select`** | `--select from,to,value` | No `-g`; **≥3 columns** in order source, target, value[, extra measures] |

```bash
# Group path (source=from, target=to)
vizb sankey examples/csv/sankey-flows.csv -g from,to -p x,y -o out.html

# Solo --select path (same roles, same order: source, target, value)
vizb sankey examples/csv/sankey-flows.csv --select from,to,value -o out.html

# Solo multi-measure → stat tabs (source, target, then measures)
vizb sankey examples/csv/sankey-flows.csv --select from,to,value,cost -o out.html
```

Solo `--select` with only 1 or 2 columns is **rejected** for Sankey. The first two columns are always treated as node names (even if they look numeric).
