...
Map columns with **group** (same model as heatmap) or **solo `--select`**:

```bash
# Group: from → to; numeric columns become measures
vizb sankey data.csv -g from,to -p x,y -o out.html

# Solo --select (no -g): at least 3 columns — source, target, value
vizb sankey data.csv --select from,to,value -o out.html
```
...
| Approach | Flags | When to use |
|----------|--------|-------------|
| **Group** | `-g from,to -p x,y` | Category axes via group pattern; all numeric columns become measures |
| **Solo `--select`** | `--select from,to,value` | No grouping; **at least 3 columns**: source, target, value[, extra measures] |

```bash
# Group path
vizb sankey examples/csv/sankey-flows.csv -g from,to -p x,y -o out.html

# Solo --select path (min 3 columns)
vizb sankey examples/csv/sankey-flows.csv --select from,to,value -o out.html

# Solo multi-measure → stat tabs
vizb sankey examples/csv/sankey-flows.csv --select from,to,value,cost -o out.html
```

Solo `--select` with only 1 or 2 columns is **rejected** for Sankey. Source and target are always categorical node names (even if they look numeric).
...
```bash
vizb sankey examples/csv/sankey-flows.csv -g name,from,to -p n,x,y -o out.html
```
...
vizb sankey data.csv -g from,to,channel -p x,y,z -o out.html
```
...
vizb sankey examples/csv/sankey-flows.csv -g from,to -p x,y --select value -o out.html
```
...
vizb sankey examples/csv/sankey-flows.csv -g from,to -p x,y -l --sort desc -o out.html

# Root command: opt-in renderer only
vizb examples/csv/sankey-flows.csv -g from,to -p x,y -c sankey -o out.html
```
...
vizb data.csv -g from,to -p x,y -c sankey -o out.html
vizb data.csv -g from,to -p x,y -c bar,sankey -o out.html
```
