...
vizb sankey data.csv --select source,target,value -o out.html
```
...
| **Solo `--select`** | `--select source,target,value` | No `-g`; **≥3 columns** in order source, target, value[, extra measures] |

```bash
# Group path (columns source → target)
vizb sankey examples/csv/sankey-flows.csv -g source,target -p x,y -o out.html

# Solo --select path (same column order: source, target, value)
vizb sankey examples/csv/sankey-flows.csv --select source,target,value -o out.html

# Solo multi-measure → stat tabs
vizb sankey examples/csv/sankey-flows.csv --select source,target,value,cost -o out.html
```
