...
import sankeyChart from '../../../assets/sankey-chart.png';
import { SANKEY_FLOWS_SAMPLE } from '../../../data/samples';
...
<CopyableCsv
  filename="sankey-flows.csv"
  csv={SANKEY_FLOWS_SAMPLE}
/>

Group: list source then target under `-g`; `-p x,y` assigns those roles:

<InvokeTabs
  cli={`vizb sankey sankey-flows.csv -g source,target -p x,y -o out.html`}
  input={SANKEY_FLOWS_SAMPLE}
/>

Solo `--select`: exactly 3 columns — source, target, value (no `-g`):

<InvokeTabs
  cli={`vizb sankey sankey-flows.csv --select source,target,value -o out.html`}
  input={SANKEY_FLOWS_SAMPLE}
/>

<DocImage src={sankeyChart} alt="Sankey chart of multi-hop flows with weighted links between source and target nodes" />
...
<InvokeTabs
  cli={`vizb sankey sankey-flows.csv \
...
<InvokeTabs
  cli={`vizb sankey sankey-flows.csv -g name,source,target -p n,x,y -o out.html`}
  input={SANKEY_FLOWS_SAMPLE}
/>
