...
<CardGrid>
  ...
  <Card title="Chord Chart" icon="random" href="/charts/chord">
...
To override a setting for a single chart type without affecting the others, use the `--chart <type>:key=val` syntax. Object props use `{field=value;field=value}` (semicolons inside braces; commas stay literal). Unwrapped `bar:bg=color=…` or `line:scale=type=log;axes=x` is invalid. Nested `log:axes=x` is not valid.
...
# Per-axis log on line
vizb data.csv -c line --chart 'line:scale={type=log;axes=x}' -o output.html
```
...
<InvokeTabs
  cli={`vizb data.csv -g category,metric -p x,y -c bar,radar -o output.html`}
/>

<InvokeTabs
  cli={`vizb data.csv -g source,target -p x,y -c sankey -o output.html`}
/>

<InvokeTabs
  cli={`vizb data.csv -g source,target -p x,y -c chord -o output.html`}
/>

<InvokeTabs
  cli={`vizb data.csv -g category -c bar -o output.html`}
/>
