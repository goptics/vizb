...
Most settings apply to every chart type. The exceptions are `scale` (log), which only affects cartesian charts, and `rotate` (3D auto-spin), which only applies when bar or line renders in WebGL mode.
...
| `swap` (axis reorder) | ✓ | ✓ | ✓ | ✓ | ✓ |
| `scale` log | ✓ | ✓ | ✗ | ✗ | ✗ |
| `rotate` (3D auto-rotate) | ✓ 3D only | ✓ 3D only | ✗ | ✗ | ✗ |
