...
Vizb folds the same **x/y/z dimension model** into every chart type — you describe your data once with a group pattern (e.g. `-p x,y,z` for CSV or `-p x/y/z` for benchmarks), and all five renderers interpret it in parallel. One pipeline, five different shapes. Use `--charts` (or `-c`) to control which renderers are bundled into the output file.
