...
<Aside type="note">
  Heatmap and Sankey require at least a 2D pattern (`-p x,y` for CSV or `-p x/y` for benchmarks). Passing `-p x` with `-c heatmap` or `-c sankey` will produce an empty chart panel. For Sankey, treat the edge list as source/target columns (e.g. `-g source,target -p x,y`).
</Aside>
