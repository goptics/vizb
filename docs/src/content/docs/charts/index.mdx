...
<CardGrid>
  ...
  <Card title="Sankey Chart" icon="seti:pipeline" href="/charts/sankey">
...
| `-p x,y,z` (CSV) or `-p x/y/z` (bench) | **3D** — X + Y + Z | Z adds a third dimension; bar/line go WebGL, pie/radar add a third slice set, heatmap blends Z into cell color; **Sankey ignores z** |

Pie, radar, and Sankey have no cartesian axes in the usual sense, but the same `-p` pattern governs how they interpret dimensions:
...
{/* TODO: Add GIF showing chart types rendered side by side */}

<Aside type="note">
  Heatmap and Sankey require at least a 2D pattern (`-p x,y` for CSV or `-p x/y` for benchmarks). Passing `-p x` with `-c heatmap` or `-c sankey` will produce an empty chart panel. For Sankey, treat the edge list as source/target columns (e.g. `-g from,to -p x,y`).
</Aside>
...
Most settings apply to every chart type. The exceptions are `scale` (log), which only affects cartesian charts, and `3d-rotate` (3D auto-spin), which only applies when bar, line, or scatter renders in WebGL mode. Sankey v1 supports sort, labels, and swap only.

| Setting | `bar` | `line` | `scatter` | `pie` | `radar` | `heatmap` | `sankey` |
|---------|:-----:|:------:|:---------:|:-----:|:-------:|:---------:|:--------:|
| `sort` / `--sort` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `labels` / `--show-labels` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| `swap` (axis reorder) | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
...
Add `scatter`, `heatmap`, `radar`, or `sankey` explicitly when you need them: `-c bar,line,pie,scatter,heatmap,radar,sankey`.
...
# Sankey only (edge list: x = source, y = target)
vizb data.csv -g from,to -p x,y -c sankey -o output.html
