...
<InvokeTabs
  cli={`vizb bar sales.csv -g region,product,category -p x,y,z -o output.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb bar data.csv -g category,metric,group,series --group-regex "(?<n>.*)/(?<x>.*)/(?<y>.*)/(?<z>.*)" -o output.html`}
/>
...
<InvokeTabs
  cli={`vizb bar sales.csv -g region,category -p x,y --3d -o output.html`}
  input={SALES_SAMPLE}
/>

<InvokeTabs
  cli={`vizb line sales.csv -g region,category -p x,y --3d -o output.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb bar spiral.csv -o bar.html`}
/>
