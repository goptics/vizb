...
vizb supports two 3D modes for **bar**, **line**, and **scatter** charts. Both render in a WebGL scene you can rotate, zoom, and pan in the browser. Other chart types (pie, radar, heatmap) receive z-axis data differently — they don't render a 3D scene.

**Scatter** adds a third path: continuous 3D from `--axes x,y,z` (no grouping columns). See the [Scatter Chart](/charts/scatter) page for value and hybrid modes.
...
| **Value** | `vizb bar` / `vizb line` / `vizb scatter` with `--3d` on x+y-only data | categories | categories (former 2D legend) | metric value (single series, visualMap) |
...
<CardGrid>
  ...
  <Card title="3D Scatter" icon="magnifier">
    `scatter3D` points — grouped z series, continuous `--axes x,y,z`, or hybrid
    categorical x/y with a numeric z column.
  </Card>
</CardGrid>
