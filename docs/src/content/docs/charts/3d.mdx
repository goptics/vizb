...
When your CSV/JSON data has 3+ numeric columns, vizb auto-value detects them as `x`, `y`, `z` coordinates and renders continuous 3D — no `--3d` flag needed. This works on bar, line, and scatter:
...
Use `--select` to pick specific columns. With auto-value, 3D is automatically enabled when 3 coordinate columns are detected; `--select` limiting to 2 columns keeps it 2D.
