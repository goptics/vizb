...
```bash
vizb data.csv -g category -c radar -o out.html
```
...
```bash
vizb data.csv -g category,metric -p x/y -c radar -o out.html
```
...
```bash
vizb data.csv -g category,metric,group -p x/y/z -c radar -o out.html
```
...
```bash
# Sort and show labels on the radar only
vizb data.csv -g category,metric --chart radar:sort=asc,labels -o out.html
```
