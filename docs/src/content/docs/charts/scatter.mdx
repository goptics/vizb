...
import { SALES_SAMPLE, REGION_METRICS_SAMPLE } from '../../../data/samples';
...
<InvokeTabs
  cli={`vizb scatter sales.csv -g order_date -p x -o out.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb scatter sales.csv -g region,category -p x,y -o out.html`}
  input={SALES_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb scatter sales.csv -g order_date,category -p "[-y{Month}-x{Date}],z{Category}" -o out.html`}
  input={SALES_SAMPLE}
/>
...
<CopyableCsv
	...
	csv={REGION_METRICS_SAMPLE}
/>

<InvokeTabs
  cli={`vizb scatter region-metrics.csv --select region,latency -o mixed.html`}
  input={REGION_METRICS_SAMPLE}
/>
...
<InvokeTabs
  cli={`vizb scatter clusters.csv --visualmap --symbol-size 10 -o scatter.html`}
/>
...
| Scale (log) | `--scale log` or `--scale 'type=log;axes=x'` | Scale toggle | `linear` / `log` (value axis), or bag for per-axis log — 2D grouped, value, and mixed mode. Panel cannot pick axes or base; hover is read-only |
...
vizb data.csv -c scatter --chart 'scatter:scale={type=log;axes=x}' -o out.html
