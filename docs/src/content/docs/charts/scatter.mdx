...
The 1D example uses [`sales.csv`](https://github.com/goptics/vizb/blob/main/sales.csv) — 10,000 order rows with categorical columns (`order_date`, `region`, `category`, …) and numeric metrics (`quantity`, `amount`, `total`, …). You can also browse [live CSV dashboards](https://vizb.goptics.org/examples/csv) built from the same file.
...
<Image src={scatter1D} alt="1D grouped scatter chart plotting quantity per order date" />
...
<Image src={scatter2D} alt="2D grouped scatter chart with five category series per region and a color category legend" />
...
<Image src={scatter3D} alt="3D stacked scatter chart with date, month, and category axes in a WebGL scene" />
...
<Image src={scatter3DMixed} alt="3D mixed scatter chart with country on the category axis and life expectancy and income on value axes" />
...
<Image src={scatter2DValue} alt="2D value-mode scatter chart of cluster coordinates with visualMap gradient coloring" />
