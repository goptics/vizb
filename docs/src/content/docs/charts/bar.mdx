...
| Border radius | `--border-radius <int>` | — | Corner radius in px on free outer ends; stacked bars round only the top segment — 2D only (CLI/config; not in UI settings) |
...
# Rounded free outer corners (stacked: top segment only)
