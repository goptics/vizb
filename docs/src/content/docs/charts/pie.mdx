...
<Image src={pie1D} alt="1D pie chart showing quantity share by region with labeled slices" />
...
<Image src={pie2D} alt="2D pie charts side by side — quantity by region and by category" />
...
<Image src={pie3D} alt="3D pie charts in a row — quantity by date, month, and category" />
