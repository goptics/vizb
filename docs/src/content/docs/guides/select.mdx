...
- **Sankey** solo `--select` needs **at least 3 columns**, in the same order as group: **source, target, value** (then optional extra measures). Example: `--select source,target,value` matches `-g source,target -p x,y` plus measure `value`. One or two columns are rejected. The first two columns are always node names; remaining columns are numeric stats (tabs when multiple).
- **Repeatable** `--select` takes 2 columns per flag only: each flag is `dim,metric` → one stat type in a **single dataset**; charts separate by stat name. (Legacy: `{label}` on the metric column still overrides the stat name when `()` is omitted.) Sankey does not use repeatable solo `--select` — put extra measures on one flag after the source and target columns.
