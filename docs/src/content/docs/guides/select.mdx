...
- **Sankey** solo `--select` needs **exactly 3 columns** per flag: **source, target, value** (same order as group). Example: `--select source,target,value` matches `-g source,target -p x,y` with measure `value`. One, two, or four+ columns on one flag are rejected.
- **Repeatable** `--select`: for scatter/multi-stat, **2 columns** per flag (`dim,metric`). For Sankey multi-measure, **3 columns** per flag with the same source/target (e.g. `--select source,target,value --select source,target,cost`). Charts separate by stat name. (Legacy: `{label}` on the metric column still overrides the stat name when `()` is omitted.)
