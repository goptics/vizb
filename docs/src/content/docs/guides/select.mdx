...
- **Sankey** solo `--select` needs **at least 3 columns**: `source,target,value[,metrics...]` (no `-g`). One or two columns are rejected. Source and target are always node names; measures become stats (tabs when multiple).
