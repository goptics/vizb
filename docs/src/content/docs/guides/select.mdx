...
- **Sankey** solo `--select` needs **at least 3 columns**, in the same order as group: **source, target, value** (then optional extra measures). Example: `--select from,to,value` matches `-g from,to -p x,y` plus measure `value`. One or two columns are rejected. The first two columns are always node names; remaining columns are numeric stats (tabs when multiple).
