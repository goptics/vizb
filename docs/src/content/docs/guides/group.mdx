...
<Aside type="note">
  At least one axis (`x` or `y`) is required for group to take effect. The `name` dimension is optional. When omitted, all rows go into a single chart. The `z` dimension requires **both** `x` and `y` to be present.
</Aside>
