...
Merging combines multiple vizb dataset JSON files into one — letting you overlay results across runs, machines, or releases in a single interactive chart. Benchmarks are the common case, but any dataset vizb produces (including charted CSV/JSON) merges the same way.

Without merging, each run is an isolated report. With merging, you can overlay results from `v1.0` and `v2.0`, compare performance across different machines, or track regressions over time.
