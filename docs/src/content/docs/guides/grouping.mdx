...
When grouping is active (`--group` or auto-group), multiple rows can share the same `(name, x, y, z)` key. Vizb **sums** their values into a single point. A 200k-row export collapses into a few thousand chart points. You'll see it in the CLI output when duplicates were merged:
...
When every row already has a unique group key (e.g. auto-group picked a high-cardinality ID column), vizb still runs the sum step but nothing collapses. The opening line is the same; the closing line notes that every key was unique:
