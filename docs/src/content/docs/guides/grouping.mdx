...
|---|---|---|
| 2+ | `x`, `y` | 2D value |
| 3+ | `x`, `y`, `z` (first 3 only; extras ignored) | **3D value** (auto-enables 3D) |

With 2+ numeric columns, vizb creates a 2D value-mode chart. With 3+, it automatically enables 3D (`bar3D` / `line3D` / `scatter3D`). The inference is logged: `🧠 Auto-valued by columns: price, latency, memory (3D pattern x-y-z)`.

Use `--select` to control which columns participate. When `--select` limits to 2 columns, auto-value stays 2D even if the data has more numeric columns:
...
vizb scatter allnums.csv --select x,y -o 2d.html # --select limits to 2D
