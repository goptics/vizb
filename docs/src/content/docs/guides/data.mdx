...
Vizb charts any tabular data. The `csv` and `json` parsers turn a table into the same interactive 4D charts. Each numeric column/field becomes a chart series. Non-numeric columns can become the Name / X / Y dimensions with `--group`.
