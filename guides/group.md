...
```bash
vizb bar sales.csv -g region,product -p y,x -o sales.html
```
...
| Goal | Jump to |
|------|---------|
| Decide group vs select | [Group vs Select](/guides/group-vs-select) |
| What n / x / y / z mean | [Dimensions](/getting-started/dimensions) |
| Separators, brackets, regex, labels | Sections below on this page |

## What is group

Group turns each row into a point with up to four dimensions (**Name**, **X**, **Y**, **Z** — see [Dimensions](/getting-started/dimensions)). That structure is what makes multi-axis charts possible.
...
Without group, every row is an isolated bar or point. With it, vizb can:
...
> At least one of `x` or `y` is required for group to take effect. `n` is optional (one chart when omitted). `z` requires both `x` and `y`. Full rules: [Dimensions](/getting-started/dimensions).
...
|---|---|---|---|
| Comma list (`-g region,product,category`) | Commas (`-p x,y,z`) | `Asia,Widget,Electronics` | `-p x/y/z` → `expected ", ,", got "/ /"` |
| Quoted string (`-g "region product"`) | Spaces (`-p "y x"`) | `UAE Gadget` | — |
| Single value (`-g "region/product/category"`) | Same separators (`-p x/y/z`) | `EU/Widget/Sports` | `-p x,y/z` → `expected "/ /", got ", /"` |
...
For the first row, vizb joins the selected columns into the label `Asia,Widget`, then the pattern `y,x` splits it: `y` → `Asia` (YAxis), `x` → `Widget` (XAxis).
...
**ISO dates (`YYYY-MM-DD`)** — use a bracket slot when the date is one column. `{year}`, `{months}`, and `{dates}` only set axis titles. Splitting is driven by `-` between dimension tokens. The same prefix/middle/trailing skip rules from [Separators](#separators) apply inside brackets too — e.g. `[n{year}-x{months}-]` drops the day, `[n{year}--x{dates}]` skips the month.
...
100,3103.62,3103.73,3104.59,3103.72,3114.20,3105.10
1000,32423.67,32421.28,32413.72,32408.85,32411.78,32413.10
```
